package com.etone.project.core.db.conn;
...
/**
 * Iq数据库连接池 单例模式 使用c3p0连接池
 * Created with IntelliJ IDEA.
 * User: Guojian
 * Date: 14-9-01
 * Time: 上午11:18
 */
public final class IqConnectionManager {

    private static IqConnectionManager instance;
    private ComboPooledDataSource ds;

    private IqConnectionManager() throws Exception {

        ds = new ComboPooledDataSource();
        ds.setDriverClass(Global.IQ_CONN_POOL_DRIVER);
        ds.setJdbcUrl(Global.IQ_CONN_POOL_URL);
        ds.setUser(Global.IQ_CONN_POOL_USER);
        ds.setPassword(Global.IQ_CONN_POOL_PASSWORD);
        try {
            int initialPoolSize = Global.IQ_CONN_POOL_INIT_CONN_NUM;
            // 初始化时获取连接，取值应在minPoolSize与maxPoolSize之间。Default: 3 initialPoolSize
            ds.setInitialPoolSize(initialPoolSize);
            int maxPoolSize = Global.IQ_CONN_POOL_MAX_CONN_NUM;
    ...
    public static final IqConnectionManager getInstance() {
        if (instance == null) {
            try {
                instance = new IqConnectionManager();
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
