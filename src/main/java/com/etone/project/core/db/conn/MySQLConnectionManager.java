...
public final class MySQLConnectionManager {
    ...
    private MySQLConnectionManager() {
        ...
        ds.setDriverClass(Global.MYSQL_CONN_POOL_DRIVER);
        ds.setJdbcUrl(Global.MYSQL_CONN_POOL_URL);
        ds.setUser(Global.MYSQL_CONN_POOL_USER);
        ds.setPassword(Global.MYSQL_CONN_POOL_PASSWORD);

            int initialPoolSize = Global.MYSQL_CONN_POOL_INIT_CONN_NUM;
            // 初始化时获取连接，取值应在minPoolSize与maxPoolSize之间。Default: 3 initialPoolSize
            ds.setInitialPoolSize(initialPoolSize);
            int maxPoolSize = Global.MYSQL_CONN_POOL_MAX_CONN_NUM;
    ...
    public static final MySQLConnectionManager getInstance() {
        if (instance == null) {
            try {
