/**
 * bootstrap的扩展
 * @author zhangjun 448247895@qq.com
 * @version 2016-07-18
 */
...
bs.addTabs = function (options) {
    if(options.id!=undefined && options.id.substr(0,13)=="parentAddtabs"){
        ...
        /* 20161209把本if判断中的parent全部改成 parent.window.parent。【可以嵌套两层ifame，即tab页中的jsp再ifame的jsp中添加tab页。】
           是为了解决嵌套两层的ifame无法添加tab页的问题 。但仅仅可以嵌套两层，   */
        var active_flag = false;
        if($("#" + id)){
            active_flag = parent.window.parent.$("#" + id).hasClass('active');
        }
        parent.window.parent.$(".active").removeClass("active");
        //如果TAB不存在，创建一个新的TAB
        if (!parent.window.parent.$("#" + id)[0]) {
            //固定TAB中IFRAME高度
            mainHeight = parent.window.parent.$(document.body).height() - 200;
            //创建新TAB的title
            title = '<li id="tab_' + id + '"><a href="#' + id + '" role="tab" data-toggle="tab">'+options.title;
            ...
            if (options.content) {
                ...
                content = '<div role="tabpanel" class="tab-pane active" id="' + id + '" style="height:100%"><iframe id="iframe_'+id+'" src="' + options.url +
                    '" width="100%" height="100%" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="yes" allowtransparency="yes"></iframe></div>';
            }
            //加入TABS
            parent.window.parent.$(".nav-tabs").append(title);
            parent.window.parent.$(".tab-content").append(content);
        }else{
            if(active_flag){
                parent.window.parent.$("#iframe_" + id).attr('src', parent.window.parent.$("#iframe_" + id).attr('src'));
                //$("#iframe_" + id).attr('src', $("#tab_" + id).attr('src'));
            }
        }
        //激活TAB
        parent.window.parent.$("#tab_" + id).addClass('active');
        parent.window.parent.$("#" + id).addClass("active");
    }else{
