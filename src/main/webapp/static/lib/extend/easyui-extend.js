...
//扩展$.messager,通过$.messager.progress改进，去除了进度条
$.extend($.messager, {
    loading : function(methodName) {
        var method = {
            close : function() {
        ...
        win.append("<div class=\"messager-progress\"><div class=\"messager-p-msg\"></div></div>");
        win.window({
            title : opts.title,
...
eu.loading = function() {
    $.messager.loading({
        msg: '<div class="loadingImg">正在处理，请稍候……</div>'
    });
}
...
/**
 *  zj
 *  Datagrid 表格 rownumbers行号四位、五位显示不完全的解决办法
 *  使用：
 *   $("#easyui-datagrid").datagrid({
 *      onLoadSuccess : function (data) {
 *          $(this).datagrid("fixRownumber"); //解决行号rownumbers 超过三位显示不全的问题
 *      }
 *    });
 */
$.extend($.fn.datagrid.methods, {
