var writeInfo = {
    ...
    init : function() {
       ...
       // }
		...
		this.currentDate = new Date((+new Date())-0*24*3600*1000).Format("yyyy-MM-dd");
        ...
        $("input[id^='projectCode']").each(function(index, element) {
            $(this).typeahead({
                source: function (query, process) {
					//query是输入的值
					$.post(that.action.queryContractReview, { name: query }, function (datas) {
						var array = [];
    ...
    events : {
        ...
        reset : function() {
			...
			$("#logTime").val(new Date().Format("yyyy-MM-dd"));
			$("#workNature").val("");
			$("#context").val("");
